...
#Data Download Tools: These packages come with the VM
  ...
  echo -n "Minpath: "; head -n3 /opt/MinPath/MinPath.py | tail -1 | sed 's/MinPath/\/opt\/MinPath\/MinPath.py/'| sed 's/current //' | sed 's/[^(]*//'
  ##Qiime and Metaphlan
  echo -n "QIIME: "; qiime --version 2>&1 | head -1 | sed 's/[^8.]*//'
